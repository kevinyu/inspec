...
class ExampleThreadedLiveAudioApp(InspecCursesApp):
    def __init__(
            self,
            device,
            duration=1,
        ...
        self.fft_win = np.hanning(self.chunk_size)
    ...
    def start_tasks(self):
        ...
        asyncio.create_task(self.render_psd_output())

    def psd(self, chunk):
        # normalized, windowed frequencies in data chunk
        scaled = db_scale(chunk[:, [0]], dB=0)

        spec = np.fft.rfft(scaled * self.fft_win) / self.chunk_size
        psd = abs(spec) ** 2           # magnitude
        psd += 1e-5                    # Add fake noise floor
        psd = 20 * np.log10(psd)       # to dB scale

        window_size = self.pad.getmaxyx()
        desired_size = HalfCharMap.max_img_shape(window_size[0], 1)

        max_chunk = np.max(scaled)
        max_max = 1000
        img = np.zeros(desired_size[0])
        img[:int(len(img) * max_chunk / max_max)] = 1

        char_array = HalfCharMap.to_char_array(img[:, None], floor=0, ceil=1)
        colorized_char_array = CursesRenderer.apply_cmap_to_char_array(self.cmap, char_array)
        self.psd_queue.put_nowait(colorized_char_array)
    ...
    async def render_psd_output(self):
        """This is called by matplotlib for each plot update.
        ...
        """
        while True:
            colorized_char_array = await self.psd_queue.get()
            CursesRenderer.render(
                self.pad,
                colorized_char_array,
                start_col=self.draw_at
            )
            CursesRenderer.render(
                self.pad,
                colorized_char_array,
                start_col=self.duplicate_at
            )
            self.write_column(self.erase_at, " ")
            self.state["current_x"] += 1
            self.state["current_x"] = self.state["current_x"] % (self.width + 1)
